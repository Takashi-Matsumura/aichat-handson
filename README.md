...
| その他 | `iogpu.wired_limit_mb`はOS既定値のまま（変更なし）。`--cache-ram`は未指定（llama-serverの既定値8192MiB） |
...
で `scripts/llama-launchd.sh apply 50` を実行すれば当日と同じ並列数・コンテキスト長になります
（その後の変更で`--cache-ram 2048`が追加されている点だけが当日と異なります）
（研修がない期間は並列4に縮小して運用しているため、研修前に必ず実行してください）。
...
> **現在の運用（2026-09-23〜）: 研修がない期間は `--parallel 4`（`--ctx-size 16384`）で運用中です。**
> メモリ使用量を抑えるため、両モデルとも `scripts/llama-launchd.sh apply 4` で縮小しています
> （RSS: gemma-4-E4B-it ≒ 6.14 GiB、gemma-3n-E4B-it ≒ 4.31 GiB、合計 約10.5 GiB）。
> 開発・動作確認で数名が同時に使う程度なら問題ありません。**研修を行う場合は前日までに
> `scripts/llama-launchd.sh apply 50`（または参加人数に合わせた値）で元に戻してください。**
...
- チャット用の2台には `--cache-ram 2048`（プロンプトキャッシュの上限、1台あたり2GiB）を指定しています。
  llama-serverの既定値は8192MiBで、長時間・多人数で使うと1台あたり最大8GiBまでメモリが増えるためです。
  このキャッシュはGPUではなく通常のメモリに置かれるので、見積もりでは「+プロンプトキャッシュ」として
  別の行に表示します。変更する場合は `CACHE_RAM_MIB=4096 scripts/llama-launchd.sh apply 50` のように
  指定してください（`0`でキャッシュ無効、`-1`で上限なし）
